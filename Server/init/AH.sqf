...
_PV_DevUlDs = [];comment 'Those IDs (player) are hidden in the Admin-Log and Playerlist of the AdminMenu but have SuperAdmin access';
PV_ADMIN_DONT_LOG = [];comment 'Those IDs (player) are hidden in the Admin-Log';
...
if((preProcessFileLineNumbers ("devs.sqf")) != "")then{
	_tmpDV = call compile preProcessFileLineNumbers "devs.sqf";
	if(!isNil '_tmpDV')then
	{
		if(typeName _tmpDV == 'ARRAY')then
		{
			if(str _tmpDV != '[]')then
			{
				_PV_DevUlDs = _PV_DevUlDs + _tmpDV;
			};
		};
	};
};
//Admin Loadout Variable to be called from ESSv2
AdminLoadout = ["0"];publicVariable "AdminLoadout";
AdminLoadout = AdminLoadout + _LAdmins + _NAdmins + _SAdmins + _PV_DevUlDs;publicVariable "AdminLoadout";

//Admin Loadout Variable to be called from ESSv2
MasterKey = ["0"];publicVariable "MasterKey";
MasterKey = MasterKey  + _NAdmins + _SAdmins + _PV_DevUlDs;publicVariable "MasterKey";

/**DoorManagement Config**/
DoorAdminList = MasterKey; // List of Player Id's of admins that can manage all doors

iproductVersion = '06-Jul-2016 01-08-21-v1431A';
...
_fnc_RandomGen =
{
	...
	_arr = ['h','2','n','7','t','x','y','l','b','c','R','B','c','d','0','K','F','I','8','F','4','u','U','I','L','9','o','U','y','q'];
...
_randvar217037 = call _fnc_RandomGen;diag_log format['infiSTAR.de - _randvar217037: %1',_randvar217037];
...
_randvar217010 = 'PVAHR_0_' + (call _fnc_RandomGen);diag_log format['infiSTAR.de - _randvar217010: %1',_randvar217010];
...
{
	[] spawn {
		waitUntil {_i = '28210';!isNil 'sm_done'};
		uiSleep 30;
...
"+_randvar11+" = {
	...
	if(str (toArray 'ctrlText') != '[99,116,114,108,84,101,120,116]')then
	{
		...
		[_name,_puid,'BAN',toArray ('FNCbroken: ctrlText')] call "+_randvar217037+";
	};
	if(str (toArray 'ctrlEnabled') != '[99,116,114,108,69,110,97,98,108,101,100]')then
	{
		...
		[_name,_puid,'BAN',toArray ('FNCbroken: ctrlEnabled')] call "+_randvar217037+";
	};
	if(str (toArray 'finddisplay') != '[102,105,110,100,100,105,115,112,108,97,121]')then
	{
		...
		[_name,_puid,'BAN',toArray ('FNCbroken: finddisplay')] call "+_randvar217037+";
	};
	if(ctrlEnabled 4)then
	{
		AntiHack_CTRL4 = 'AntiHack_CTRL4';publicVariable 'AntiHack_CTRL4';
		[_name,_puid,'BAN',toArray ('Active Menu CTRL4')] call "+_randvar217037+";
	};
	if(ctrlText 8 != '')then
	{
		AntiHack_CTRL8 = 'AntiHack_CTRL8';publicVariable 'AntiHack_CTRL8';
		[_name,_puid,'BAN',toArray ('Active Menu CTRL8')] call "+_randvar217037+";
	};
	if(ctrlEnabled 810)then
	{
		AntiHack_CTRL810 = 'AntiHack_CTRL810';publicVariable 'AntiHack_CTRL810';
		[_name,_puid,'BAN',toArray ('Active Menu CTRL810')] call "+_randvar217037+";
	};
	if !(true)then
	{
		AntiHack_ValueReassignment = 'AntiHack_ValueReassignment';publicVariable 'AntiHack_ValueReassignment';
		[_name,_puid,'BAN',toArray ('Value Reassignment: true')] call "+_randvar217037+";
	};
	if(false)then
	{
		AntiHack_ValueReassignment = 'AntiHack_ValueReassignment';publicVariable 'AntiHack_ValueReassignment';
		[_name,_puid,'BAN',toArray ('Value Reassignment: false')] call "+_randvar217037+";
	};
	if(!isNil 'BIS_loadingText')then
	{
		AntiHack_loadingText = 'AntiHack_loadingText';publicVariable 'AntiHack_loadingText';
		[_name,_puid,'BAN',toArray ('loadingText')] call "+_randvar217037+";
	};
	{
		if(!isNull (findDisplay _x))then
		{
			_log = format['MenuBasedHack_D%1',_x];
			[_name,_puid,'BAN',toArray(_log)] call "+_randvar217037+";
	...
	if!(_puid in "+str _allAdmins+")then
	{
		if(!isNil 'PV_AdminMainCode')then{
			[_name,_puid,'BAN',toArray ('Admin Variable - PV_AdminMainCode - Found')] call "+_randvar217037+";
			[] spawn {sleep 0.5;AntiHack_adminvar = 'AntiHack_adminvar';publicVariable 'AntiHack_adminvar';};
		};
		if(!isNil 'PV_LowLevel_List')then{
			[_name,_puid,'BAN',toArray ('Admin Variable - PV_LowLevel_List - Found')] call "+_randvar217037+";
			[] spawn {sleep 0.5;AntiHack_adminvar = 'AntiHack_adminvar';publicVariable 'AntiHack_adminvar';};
		};
		if(!isNil 'PV_NormalLevel_List')then{
			[_name,_puid,'BAN',toArray ('Admin Variable - PV_NormalLevel_List - Found')] call "+_randvar217037+";
			[] spawn {sleep 0.5;AntiHack_adminvar = 'AntiHack_adminvar';publicVariable 'AntiHack_adminvar';};
		};
		if(!isNil 'PV_SuperLevel_List')then{
			[_name,_puid,'BAN',toArray ('Admin Variable - PV_SuperLevel_List - Found')] call "+_randvar217037+";
			[] spawn {sleep 0.5;AntiHack_adminvar = 'AntiHack_adminvar';publicVariable 'AntiHack_adminvar';};
		};
		if(!isNil 'PV_DevUlDs')then{
			[] spawn {sleep 0.5;AntiHack_adminvar = 'AntiHack_adminvar';publicVariable 'AntiHack_adminvar';};
			[_name,_puid,'BAN',toArray ('Admin Variable - PV_DevUlDs - Found')] call "+_randvar217037+";
		};
		if(!isNil 'setGroupIconsVisible')then{
			[_name,_puid,'BAN',toArray ('FNCbroken: setGroupIconsVisible')] call "+_randvar217037+";
		...
		if(!isNil 'groupIconsVisible')then{
			[_name,_puid,'BAN',toArray ('FNCbroken: groupIconsVisible')] call "+_randvar217037+";
			[] spawn {sleep 0.5;AntiHack_GroupIcons = 'AntiHack_GroupIcons';publicVariable 'AntiHack_GroupIcons';};
		};
	...
	{
		[_name,_puid,'BAN',toArray ('Active Menu: Zero')] call "+_randvar217037+";
...
[] spawn {
	...
	_zero = ""
		if(!isNull player)then
		{
			if(isServer)then
			{
				...
				[name player,getPlayerUID player,'BAN',toArray (_log)] call "+_randvar217037+";
				[] spawn "+_randvar2+";
			};
		};
					...
					[] spawn {
						while{1==1}do {
							...
							if(isNil '"+_antidupePVResVar+"')then{
								...
								[name player,getPlayerUID player,'SLOG',toArray (_log)] call "+_randvar217037+";
							};
							waitUntil {uiSleep 0.5;isNull (findDisplay 49)};
						};
						...
						[_puid] spawn {
							...
							_bye = false;
							{
								if(isNil _x)then
								{
									...
									_name = 'DEAD';if((alive player)&&(getPlayerUID player != ''))then{_name = name player;};
									[_name,_puid,'HLOG_SKICK',toArray (_log)] call "+_randvar217037+";
								};
								true
							...
							if(_bye)then{
								...
								_name = 'DEAD';if((alive player)&&(getPlayerUID player != ''))then{_name = name player;};
								[_name,getPlayerUID player,'BAN',toArray (_log)] call "+_randvar217037+";
	...
	_one = ""
		if(!isDedicated)then
		{
			...
			if((!isNil 'hasInterface') || (!isNil 'isDedicated') || (!isNil 'isServer') || (!isNil 'uiSleep'))then
			{
				...
				[] spawn "+_randvar2+";
				[_name,_puid,'BAN',toArray ('SpawnInBroken')] call "+_randvar217037+";
			};
			if(isNil '"+_randvar36+"')then
			{
				...
				_name = 'DEAD';if((alive player)&&(getPlayerUID player != ''))then{_name = name player;};
				_puid = getPlayerUID player;
				"+_randvar217010+" = [_name,_puid,'BAN',toArray (_log)];
				if(isNil '"+_randvar217010+"')exitWith {};
				"+_randvar36+" = true;
			};
			_t = 't';
	...
	_three = ""
		...
		if(hasInterface)then
		{
			...
			'NienUntoten','WhippyV4_PPAdd','shazbot','l33tMapESPLunsear','selectedPlayer','Lmenu1','fini_do_re','_reMsg','_vsh3itzu2','fini_trader','finiLog'];
			_fnc_havefun = {
...
"+_randvar1+" = {
	...
	if !(_puid in "+str _allAdmins+")then
	{
		[_name,_puid] spawn {
			...
			[_name,_puid] spawn {
				...
				if(isNil '"+_MenuChecksRunningx+"')then{
					...
					[_name,_puid,'SLOG',toArray (_log)] call "+_randvar217037+";
				};
			};
			...
			while{1 == 1}do
			{
				...
				{
					[] spawn "+_randvar2+";
					[_name,_puid,'BAN',toArray ('Active Menu: 64 ctrl 101')] call "+_randvar217037+";
				};
				if(!isNull ((findDisplay 49) displayCtrl 0))then
				{
					[] spawn "+_randvar2+";
					[_name,_puid,'BAN',toArray ('Active Menu: 49 ctrl 0')] call "+_randvar217037+";
				};
				if( (lbSelection  ((findDisplay 12) displayCtrl 1001)) select 0 == 1 && ((lbSize ((findDisplay 12) displayCtrl 1002)) > 2 ))then
				{
					[] spawn "+_randvar2+";
					[_name,_puid,'BAN',toArray ('YOLO Menu')] call "+_randvar217037+";
				};
				...
				"; if(_CUD)then{ _AHstring = _AHstring + "
					...
					if(!isNull _display)then
					{
						...
						if(_cntA < 3)then
						{
							...
							[_name,_puid,'HLOG',toArray (_log)] call "+_randvar217037+";
						};
							...
							if(r_player_blood == 12000)then
							{
								[] spawn {
									...
									if(r_player_blood == 12000)then
									{
										...
										_name = 'DEAD';if((alive player)&&(getPlayerUID player != ''))then{_name = name player;};
										[_name,getPlayerUID player,'SLOG',toArray (_log)] call "+_randvar217037+";
					...
					if(_sPUID != '')then
					{
						...
						if((_dist > 5000) && (_damage > 0.2))then
						{
							...
							if((_dist1 > 2500) && (_dist2 > 2500) && (_sPUID != '') && (getPlayerUID player != '') && (alive player) && (!deathHandled))then
							{
								...
								[_sNAME,_sPUID,'HLOG',toArray (_log)] call "+_randvar217037+";
							};
						};
					};
				};
					...
					if(_maxAmmo > 1)then
					{
						[_maxAmmo,_cwep,_cmag] spawn {
							...
							if(_camm == _maxAmmo)then
							{
								...
								_name = 'DEAD';if((alive player)&&(getPlayerUID player != ''))then{_name = name player;};
								[_name,getPlayerUID player,'HLOG',toArray (_log)] call "+_randvar217037+";
							};
						};
					};
		...
		[_puid,_name] spawn {
			...
			while{1 == 1}do
			{
				...
				if !("+str _UCS+")then
				{
					...
					{
						[] spawn "+_randvar2+";
						[_name,_puid,'BAN',toArray ('dynamicText CHECK 1')] call "+_randvar217037+";
					};
					if(str(uiNameSpace getVariable 'BIS_dynamicText') != 'No Display')then
					{
						[] spawn "+_randvar2+";
						[_name,_puid,'BAN',toArray ('dynamicText CHECK 2')] call "+_randvar217037+";
					};
				};
				...
				if("+str _UZC+")then
				{
					if(_noSafeZone)then
					{
						...
						{
							[] spawn "+_randvar2+";
							[_name,_puid,'BAN',toArray ('zombieCheck broken')] call "+_randvar217037+";
						}
						else
						{
							if(str player_zombieCheck != str _zombieCheck)then
							{
								...
								{
									[] spawn "+_randvar2+";
									[_name,_puid,'HLOG_SKICK',toArray ('zombieCheck changed')] call "+_randvar217037+";
								};
							};
						};
						...
						{
							[] spawn "+_randvar2+";
							[_name,_puid,'BAN',toArray ('zombieAttack broken')] call "+_randvar217037+";
						}
						else
						{
							if(str player_zombieAttack != str _zombieAttack)then
							{
								...
								{
									[] spawn "+_randvar2+";
									[_name,_puid,'HLOG_SKICK',toArray ('zombieAttack changed')] call "+_randvar217037+";
								};
							};
						};
					};
								...
								if((getPlayerUID _object == '') && (player distance _mPos > 500) && (_object distance _mPos > 500))then
								{
									...
									if(!isNull _object)then
									{
										if(isNil 'dayz_firstGroup')then
										{
											...
											_name = 'DEAD';if((alive player)&&(getPlayerUID player != ''))then{_name = name player;};
											[_name,getPlayerUID player,'HLOG_SKICK',toArray (_log)] call "+_randvar217037+";
										};
		...
		[] spawn {
			...
			{((findDisplay 12) displayCtrl 51) ctrlRemoveAllEventHandlers _x;} forEach ['MouseButtonClick','MouseButtonDown','MouseButtonUp','Draw'];
					...
					if(_distance1 > _maxdist)then
					{
						...
						if((str _lastVeh == str _curVeh) && {(player == _driver) || (isNull _driver)})then
						{
							...
							if(str "+_AHpos+" != '[]')then
							{
								...
								[name player,getPlayerUID player,'HLOG',toArray (_log)] call "+_randvar217037+";
							}
							else
							{
								...
								else
								{
									if(_TPC > 1)then
									{
										...
										[name player,getPlayerUID player,'HLOG_SKICK',toArray (_log)] call "+_randvar217037+";
									};
			...
			if("+str _UFS+")then{
				[] spawn {
					...
					[
						...
						'ArmA2OA_Extras\ben_ex.sqf','Binocular\Ahornblas.sqf','VirusXD\virusxdmain.sqf','Ahornblas.sqf','virusxdmain.sqf',
						'A2MenuV2\ExecME.sqf','Expansion\Addons\ui_addons.pbo','Negro\stompthepedal.sqf','stompthepedal.sqf',
						'\z\addons\ui_addons\decrypt.sqf',
						'ui_addons\decrypt.sqf',
						'decrypt.sqf',
						'testest.sqf','testestt.sqf','RooslahVeeSevan.sqf','localBox.sqf','News.sqf',
						'Whippy\playerTools.sqf','whippyv4\executev4.sqf','meme.sqf',
						'NovaHack.sqf','blah.sqf','Infinity_Menu\InfinityMenuV1.sqf','InfinityMenuV1.sqf','iFuckingLoveMemes\wowGetMemedSonXD_init.sqf','wowGetMemedSonXD_init.sqf'
		...
		"+_randvar4+" = {
			...
			if("+str _LBK+")then
			{
				...
				_name = 'DEAD';if((alive player)&&(getPlayerUID player != ''))then{_name = name player;};
				[_name,getPlayerUID player,'SLOG_BK',toArray (_log)] call "+_randvar217037+";
			};
					...
					while{1 == 1}do
					{
						if(!isNull findDisplay 63)then
						{
							...
							if((!isNull findDisplay 55) && (_chID in [0,1]))then
							{
								...
								{
									_log = format['Voice over Side Chat (TEMP BAN) @%1',mapGridPosition player];
									[name player,getPlayerUID player,'BAN',toArray (_log)] call "+_randvar217037+";
									uiSleep 3;
									findDisplay 46 closeDisplay 0;
		...
		[_name,_puid] spawn {
			...
			while{1 == 1}do
			{
				...
				if("+str _URC+")then
				{
					...
					{
						[] spawn "+_randvar2+";
						[_name,_puid,'BAN',toArray ('NoRecoil')] call "+_randvar217037+";
					};
				...
				if((deathHandled) && (r_player_blood > 0))then
				{
					...
					if(r_player_blood > 0)then
					{
						...
						[] spawn "+_randvar2+";
						[_name,_puid,'HLOG_SKICK',toArray ('Godmode')] call "+_randvar217037+";
					};
				};
				if(isNil 'r_player_blood')then{ r_player_blood = -500; } else { if(typeName r_player_blood != 'SCALAR')then{ r_player_blood = -500;YOLO = true; }; };
				if((!deathHandled) && (r_player_blood < 0))then
		...
		if(_version < 125548)then
		{
			...
			[name player,getPlayerUID player,'SLOG',toArray (_log2)] call "+_randvar217037+";
	...
	[_name,_puid] spawn {
		...
		while{1 == 1}do
		{
			...
			{
				[] spawn "+_randvar2+";
				[_name,_puid,'HLOG_SKICK',toArray ('CharacterID is NIL')] call "+_randvar217037+";
			}
			else
			{
			...
			if !("+str _UIM+")then
			{
				...
				else
				{
					...
					if(_txt != '')then
					{
						...
						if("+str _UDN+")then
						{
							...
							{
								(_display) closeDisplay 0;
								[player,'DAY'] call "+_randvar217037+";
							};
							if(_txt in ['!vote night','/vote night','vote night','!night','/night'])then
							{
								(_display) closeDisplay 0;
								[player,'NIGHT'] call "+_randvar217037+";
							};
						};
				...
				if(!isNil 'r_player_unconscious')then
				{
					...
					else
					{
						...
						{
							_obj setVariable['infiUnconDone','0'];comment '2170';
						};
					};
				};
			};
	...
	[] spawn {
		...
		while{1 == 1}do
		{
				...
				if(!isNull _obj)then
				{
					if(typeOf _obj == 'Plastic_Pole_EP1_DZ')then
					{
						...
						if !(_obj in deleteBuildAnnounced)then
						{
							...
							[_name,_puid,'SLOG_PP',toArray (_log)] call "+_randvar217037+";
						};
					};
				};
			};
			...
			{
				[] spawn "+_randvar2+";
				[_name,_puid,'BAN',toArray ('InvisibleHack: Admin Invisibility')] call "+_randvar217037+";
			};
			if(isNil 'iMPF_remoteExecutionServer')then
			{
				[] spawn "+_randvar2+";
				[_name,_puid,'BAN',toArray ('remExServer isNil')] call "+_randvar217037+";
			}
			else
			{
	...
	<t size='1.2' font='Bitstream' align='Center' >Players Online: %12</t><br/>
	<img size='4.75' image='%4'/><br/>
	
	
	<t size='1' font='Bitstream' align='left' color='#FF0000'>Blood Left:</t><t size='1' font='Bitstream' align='right' color='#FF0000'>%2</t><br/>
	<t size='1' font='Bitstream' align='left' color='#87CEEB'>Humanity:  </t><t size='1' font='Bitstream' align='right' color='#87CEEB'>%3</t><br/>
	<br/>
	<t size='1' font='Bitstream' align='left' color='#7FFF00'>Coins:     </t><t size='1' font='Bitstream' align='right' color='#7FFF00'>%14</t><br/>
	<t size='1' font='Bitstream' align='left' color='#FFE4C4'>Bank:      </t><t size='1' font='Bitstream' align='right' color='#FFE4C4'>%15</t><br/>
	<br/>
	<t size='1' font='Bitstream' align='left' color='#00FF7F'>Zombie Kills: </t><t size='1' font='Bitstream' align='right' color='#00FF7F'>%9</t><br/>
	<t size='1' font='Bitstream' align='left' color='#7FFFD4'>Murders: </t><t size='1' font='Bitstream' align='right'  color='#7FFFD4'>%10</t><br/>
	<t size='1' font='Bitstream' align='left' color='#EE82EE'>Bandits Killed: </t><t size='1' font='Bitstream' align='right'  color='#EE82EE'>%11</>
	<br/>
	<t size='1' font='Bitstream' align='left' color='#FFBF00'>%13</t><t size='1' font='Bitstream' align='right'  color='#FFBF00'>%5h %6min</t><br/>
	<t size='1' font='Bitstream' align='left' color='#FFBF00'>FPS: </t><t size='1' font='Bitstream' align='right'  color='#FFBF00'>%8</t><br/>
	<t size='1' font='Bitstream' align='Center' color='#CC0000'>%7</t>
	<t size='.9' font='Bitstream' align='Center' color='#CC0000'>ts.149.56.107.143</t>
						"",
						_txt,
						(r_player_blood),
						_humanity,
						...
						[(player getVariable['bankMoney',0])] call BIS_fnc_numberText
						];
						uiSleep 1;
					};
				};
	...
	if !("+str _UIM+")then
	{
		...
		diag_log format['infiSTAR.de - 06-Jul-2016 01-08-21-v1431A - Successfully Loaded on Client ID2170 (%1)',time];
	}
	else
	{
		diag_log format['Successfully Loaded on Client ID2170 (%1)',time];
	};
	"+_randvar27+" = true;
...
fnc_VoteTimeServer =
{
	...
	if((LastVoteGoneThrough == 0) || ((TimeVoteCooldown + LastVoteGoneThrough) < time))then
	{
		...
		if !(_clientUID in VoteArray)then
		{
			...
			_allPlayers = playableUnits;
			_cntAll = count _allPlayers;
			_cntVoted = {getPlayerUID _x in VoteArray} count _allPlayers;
...
'"+_randvar217010+"' addPublicVariableEventHandler
{
...
_randvar19Local = {
	...
	{
		[] spawn "+_randvar2+";
		[_name,_puid,'BAN',toArray ('NAME IS BLACKLISTED')] call "+_randvar217037+";
	};
	if(_puid in "+str _BlackList+")then
	{
		[] spawn "+_randvar2+";
		[_name,_puid,'BAN',toArray ('PUID IS BLACKLISTED')] call "+_randvar217037+";
	};
	if(!isNil '"+_randvar26+"')exitWith{
		if("+_randvar26+" != _puid)then{
			...
			[_name,"+_randvar26+",'BAN',toArray (_log)] call "+_randvar217037+";
		};
	};
	"+_randvar26+" = _puid;
	...
	[] spawn {
		...
		if(dayzPlayerLogin select 8)then
		{
			...
			_name = 'DEAD';if((alive player)&&(getPlayerUID player != ''))then{_name = name player;};
			[_name,getPlayerUID player,'SLOG_NP',toArray (_log)] call "+_randvar217037+";
		};
	};
	...
	[_puid,_name] spawn {
		...
		{
			[] spawn "+_randvar2+";
			[_name,_puid,'HLOG_SKICK',toArray ('& in name breaks BattleEye..')] call "+_randvar217037+";
		};
...
_toserver =
{
	"+_randvar217010+" = _this;publicVariableServer'"+_randvar217010+"';
};
"+_randvar217037+" = _toserver;publicVariable '"+_randvar217037+"';
...
_unit setVehicleInit ""
	...
	{
		[] spawn {
			waitUntil {if(isNil'"+_randvar217037+"')then{"+_randvar217037+"='';};if(isNil'"+_randvar38+"')then{"+_randvar38+"='';};str "+_randvar217037+" != str "+_randvar38+"};
	...
	fnc_AdminReqProceed = {
		...
		if(_option == 9003)then
		{
			...
			[_dir,_pos] spawn {
				...
				'PartWoodPlywood','bulk_empty','wood_shack_kit','workbench_kit','stick_fence_kit',
				'sandbag_nest_kit','sun_shade_kit','rusty_gate_kit','outhouse_kit','light_pole_kit',
				...
				'm240_nest_kit','metal_floor_kit','cinder_wall_kit','cinder_garage_kit','cinder_door_kit',
		...
		if(_option == 9004)then
		{
			...
			[_dir,_pos,_playerObj] spawn {
				...
				clearmagazinecargoGlobal _b0x;
				_b0x setPosATL _pos;
				{_b0x addMagazineCargoGlobal [_x, 1];} forEach ['ItemVault','ItemSledgeHead','ItemSledgeHandle','30m_plot_kit','workbench_kit'];
				_b0x addMagazineCargoGlobal ['cinder_wall_kit', 6];
				_b0x addMagazineCargoGlobal ['MortarBucket', 5];
				_b0x addMagazineCargoGlobal ['CinderBlocks', 15];
				_b0x addMagazineCargoGlobal ['cinder_garage_kit', 1];
				_b0x addMagazineCargoGlobal ['ItemTankTrap', 4];
				_b0x addMagazineCargoGlobal ['ItemPole', 4];
				_b0x addMagazineCargoGlobal ['ItemComboLock', 1];
				_b0x addMagazineCargoGlobal ['ItemWoodLadder', 1];
				_b0x addMagazineCargoGlobal ['ItemWoodStairs', 1];
				_b0x addMagazineCargoGlobal ['metal_floor_kit', 3];
		...
		if(_option == 9006)then
		{
			...
			[_dir,_pos,_playerObj] spawn {
				...
				{_b0x addMagazineCargoGlobal [_x, 75];} forEach
				[
				'metal_floor_kit'
				];
