if(isServer) then {
	...
	[_mission,_position,"extreme",format["Disabled %1", _vehname],"MainHero",true] call mission_init;
	...
	[[
		...
		[(_position select 0) - 30, (_position select 1) - 30, 0],
		[(_position select 0) - 30, (_position select 1) + 30, 0]
	],"random","extreme","Bandit","Bandit",0,2,"Random","Random",_mission] call spawn_static;
	...
	if(_complete) then {
			[_crate,[5,ai_wep_sniper],[2,crate_tools_sniper],[4,crate_items_high_value],[2,crate_backpacks_large]] call dynamic_crate;
	};
