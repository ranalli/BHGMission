if(isServer) then {
	...
	[_mission,_position,"Extreme",format["Sniper Extraction %1", _vehname],"MainBandit",true] call mission_init;	
	...
	//Bmp3
	[[(_position select 0) + 75, _position select 1, 0],[(_position select 0) + 75, _position select 1, 0],50,2,"bmp3","Extreme","Hero","Hero",_mission] call vehicle_patrol;
	//Humvee Patrol
	...
	if(_complete) then {
		[_crate,[5,ai_wep_sniper],[5,crate_tools_sniper],[5,crate_items_sniper],2] call dynamic_crate;
