...
classPick = {
	...
	if (count _class > 8) then {
		...
		if ((_level == 1) && {!(_puid in _classLevel1)}) exitWith {systemChat "This class is for VIP1 only.";_go=0;};
		if ((_level == 2) && {!(_puid in _classLevel2)}) exitWith {systemChat "This class is for VIP2 only.";_go=0;};
		if ((_level == 3) && {!(_puid in _classLevel3)}) exitWith {systemChat "This class is for VIP3 only.";_go=0;};
	};
...
classPreview = {
	...
	{if !(_x in (weapons _unit)) then {_dcuwep = _x;_unit addWeapon _dcuwep;_qty=1;};} count _weps+_startWeps;
...
if !(_isPZombie) then {
	...
	_prt_0utp = "ItemWatch";
	player addWeapon _prt_0utp;_qty=1;
	...
	{_dcwep = _x;player addWeapon _dcwep;_qty=1;} count _startWeps;
	...
	{if !(_x in (weapons player)) then {_dcwep = _x;player addWeapon _dcwep;_qty=1;};} count _weps;
	...
	if (!isNull _bag) then {
		...
		{CHECK1 _qty = (_bweps select (_forEachIndex+1));CHECK2 _dcarr = [_x,_qty];_bag addWeaponCargoGlobal _dcarr;_qty=1;};} forEach _bweps;
	};
