...
if (_dikCode == 0x16) then {
	...
	if (EARPLUGS) then {
		1 fadeSound 0.25;
		((uiNamespace getVariable "EarPlugs") displayCtrl 1) ctrlSetStructuredText parseText "<t size='2' align='center' valign='middle'><img image='Scripts\Player_Hud\icons\earplugs.paa' /></t>";
	} else {
		3 fadeSound 1;
		((uiNamespace getVariable "EarPlugs") displayCtrl 1) ctrlSetStructuredText parseText "";
	};
};
//Disabled to stop a dupe method
/* surrender 
	...
	if (_canDo and !DZE_Surrender and !(player isKindOf  "PZombie_VB")) then {
		...
		if (_dropPrimary or _dropSecondary) then {
			...
			if (_dropPrimary) then {
				...
				if (_removed == 1) then {
					_wpnadgbl = _iItem;
					_item addWeaponCargoGlobal [_wpnadgbl,1];
				};
			};
