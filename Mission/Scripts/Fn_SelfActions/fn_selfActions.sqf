...
if (_inVehicle && driver _vehicle == player) then {
	...
	if (s_player_inCarlockUnlock_crtl < 0 && _hasKeyinCar) then {
		...
		if(locked dayz_myLockedVehicle) then {
			_Unlock = dayz_myLockedVehicle addAction [format[localize "STR_EPOCH_ACTIONS_UNLOCK",_text], "Scripts\custom\vehicle_lockUnlock.sqf",[dayz_myLockedVehicle, false, (_temp_keys_names select (parseNumber _carID))], 2, true, true, "", ""];
			s_player_incarlockunlock set [count s_player_incarlockunlock,_Unlock];
			s_player_inCarlockUnlock_crtl = 1;
		} else {
			_lock = dayz_myLockedVehicle addAction [format[localize "STR_EPOCH_ACTIONS_LOCK",_text], "Scripts\custom\vehicle_lockUnlock.sqf",[dayz_myLockedVehicle, true, (_temp_keys_names select (parseNumber _carID))], 1, false, true, "", ""];
...
if (!isNull cursorTarget && !_inVehicle && !_isPZombie && (player distance cursorTarget < _allowedDistance) && _canDo) then {	//Has some kind of target
	...
	_dcav = "AllVehicles";
	_isVehicle = _cursorTarget isKindOf _dcav;
	...
	if (_canDo && (speed player <= 1) && (_cursorTarget isKindOf "Plastic_Pole_EP1_DZ")) then {
		if (s_player_plotManagement < 0) then {
            _adminList = MasterKey; // Add admins to manage all plotpoles
...
//INSERTOTHER
        ...
        if(_isModular || _isModularDoor || _isDestructable || _isWreck || _isRemovable || _isWreckBuilding || _cursorTarget isKindOf "MetalFloor_Preview_DZ" ) then {
        ...
        if((_cursorTarget isKindOf "Plastic_Pole_EP1_DZ") && (_hasToolbox && "ItemCrowbar" in _itemsPlayer)) then {
			
                _findNearestPoles22 = nearestObjects [player, ["Plastic_Pole_EP1_DZ"],15];			
				_thePlot22 = _findNearestPoles22 select 0;
                _friends22 = _thePlot22 getVariable ["plotfriends", []];
                _friendUID22 = _friends22 select 0;
                _allowed22 = _friendUID22;
		if((getPlayerUID player) in _allowed22)then {
			_player_deleteBuild = true;
			_isownerof = true;
		} else {
				_player_deleteBuild = false;
	...
	if(_player_deleteBuild) then {
		if (s_player_deleteBuild < 0) then {
				s_player_deleteBuild = player addAction [format[localize "str_actions_delete",_text], "Scripts\Fixes\remove.sqf",_cursorTarget, 1, true, true, "", ""];
		};
	} else {
	...
	if(_cursorTarget == dayz_hasFire) then {
	    ...
	    //Garage
	   	if(_typeOfCursorTarget in DZE_Garage && (player distance _cursorTarget < 5)) then {
				_findNearestPoles4 = nearestObjects[player, ["Plastic_Pole_EP1_DZ"], DZE_PlotPole select 0];
				_checkme4 = count _findNearestPoles4;
				if (_checkme4 == 1 )then {
					_fuid4  = [];
					_allowed4 = [];				
					_thePlot4 = _findNearestPoles4 select 0;
					_owner4 = _thePlot4 getVariable ["ownerPUID","010"];
					_friends4 = _thePlot4 getVariable ["plotfriends", []];
					{
					_friendUID4 = _x select 0;
					_fuid4 = _fuid4 + [_friendUID4];
					} forEach _friends4;
					_allowed4 = [_owner4];
					_allowed4 = [_owner4] + _fuid4;				
					if(_owner4 == dayz_characterID || (getPlayerUID player) in _allowed4)then {
...
if((_typeOfCursorTarget in DZE_DoorsLocked)) then {
	...
	if(_typeOfCursorTarget in DZE_UnLockedStorage && _ownerID != "0" && (player distance _cursorTarget < 3)) then {
		...
		if (s_player_lockvault < 0) then {
			if(_ownerID == dayz_combination || _ownerID == dayz_playerUID) then {
				s_player_lockvault = player addAction [format[localize "STR_EPOCH_ACTIONS_LOCK",_text], "Scripts\Fixes\LockVault.sqf",_cursorTarget, 0, false, true, "",""];
			};
		};
	...
	if ((_cursorTarget isKindOf "ModularItems") || (_cursorTarget isKindOf "Land_DZE_WoodDoor_Base") || (_cursorTarget isKindOf "CinderWallDoor_DZ_Base")) then {
		...
		if (s_player_upgrade_build < 0) then {
			...
			s_player_upgrade_build = player addAction [format[localize "STR_EPOCH_ACTIONS_UPGRADE",_text], "Scripts\custom\BuildVectors\action\player_upgrade.sqf",_cursorTarget, -1, false, true, "",""];
		};
	} else {
		player removeAction s_player_upgrade_build;
		s_player_upgrade_build = -1;
	...
	if((_isDestructable || _cursorTarget isKindOf "Land_DZE_WoodDoorLocked_Base" || _cursorTarget isKindOf "CinderWallDoorLocked_DZ_Base") && (DZE_Lock_Door == _ownerID)) then {
		...
		if (s_player_downgrade_build < 0) then {
			...
			s_player_downgrade_build = player addAction [format[localize "STR_EPOCH_ACTIONS_REMLOCK",_text], "Scripts\custom\BuildVectors\action\player_buildingDowngrade.sqf",_cursorTarget, -2, false, true, "",""];
		};
	} else {
		player removeAction s_player_downgrade_build;
		s_player_downgrade_build = -1;
	...
	if(dayz_tameDogs) then {
		...
		if (_isDog && _ownerID == dayz_characterID && _isAlive) then {
			...
			player removeAction s_player_GarageDoor;
			s_player_GarageDoor = -1;
		};
	};
...
if(_nearDoorsLocked) then {
    if (s_player_manageDoor < 0) then {
        s_player_manageDoor = player addAction ["<t color='#0059FF'>Open Garage Door</t>", "Scripts\doorManagement\player_unlockDoor.sqf",[_cursorTarget], 0, false, true, "",""];
    };
} else {
        player removeAction s_player_manageDoor;
        s_player_manageDoor = -1;
    ...
    if (_inVehicle && {_typeOfCursorTarget in [,"Land_DZE_GarageWoodDoorLocked","CinderWallDoorLocked_DZ"} && {_CursorTarget distance player <= 20}) then {
        if (s_player_GarageDoor < 0) then {		 
     s_player_GarageDoor = _vehicle addAction ["<t color='#0059FF'>Open Garage Door</t>", "Scripts\doorManagement\player_unlockDoor.sqf", [_vehicle], 999, false,true,"","driver _target == _this"];
	};
} else {
		_vehicle removeAction s_player_GarageDoor;
		s_player_GarageDoor = -1;
};
*/